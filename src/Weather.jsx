...
const Weather = () => {
  ...
  const [error, setError] = useState(() => (apiKey ? null : 'API key not configured. Please add VITE_OPENWEATHERMAP_API_KEY to .env file.'));
  ...
  useEffect(() => {
    ...
    const loadWeather = async () => {
      try {
        const apiUrl = `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent('Hyderabad')}&appid=${apiKey}&units=metric`;
